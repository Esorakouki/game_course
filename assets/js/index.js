...
function updateWoolPosition(){
  wool.threeGroup.rotation.y += delta * 6;
  wool.threeGroup.rotation.z = Math.PI/2 - (floorRotation+wool.angle);
  wool.threeGroup.position.y = -floorRadius + Math.sin(floorRotation+wool.angle) * (floorRadius+50);
  wool.threeGroup.position.x = Math.cos(floorRotation+wool.angle) * (floorRadius+50);
...
function getBonus(){
  bonusParticles.mesh.position.copy(wool.threeGroup.position);
  bonusParticles.mesh.visible = true;
  bonusParticles.explose();
  wool.angle += Math.PI/2;
  //speed*=.95;
